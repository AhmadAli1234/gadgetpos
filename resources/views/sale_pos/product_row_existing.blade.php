...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		...
		<span class="text-link text-info cursor-pointer" style="font-size:12px;" data-toggle="modal" data-target="#row_edit_product_price_modal_{{$row_count}}">
			{!! $product_name !!}
	...
	<td>
		...
		@if(count($sub_units) > 0)
			...
			<select name="products[{{$row_count}}][sub_unit_id]" class="form-control sub_unit" style="padding: 0px;">
                @foreach($sub_units as $key => $value)
