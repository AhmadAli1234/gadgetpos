...
<div class="row actions-pos-btns" style="margin-top: -15px">
	...
	<div class="col-md-3">
		...
		<button type="button" class="btn btn-success no-print @if($pos_settings['disable_pay_checkout'] != 0) hide @endif" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')"><i class="fas fa-money-check-alt" aria-hidden="true"></i> @lang('lang_v1.checkout_multi_pay') </button>
	</div>
	@php
	$is_admin = auth()->user()->hasRole('Admin#' . session('business.id')) ? true : false;
	@endphp
	<div class="@if($is_admin || auth()->user()->can('access_recent_transcations')) col-md-2 @else col-md-4 @endif">
		<button type="button" class="btn btn-info no-print @if($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif pos-express-finalize" data-pay_method="cash" title="@lang('tooltip.express_checkout')"> <i class="fas fa-dollar-sign"></i> @lang('lang_v1.express_checkout_cash')</button>
	</div>
	@if($is_admin || auth()->user()->can('access_recent_transcations'))
	@if(!isset($pos_settings['hide_recent_trans']) || $pos_settings['hide_recent_trans'] == 0)	
	<div class="col-md-2">

		<button type="button" class="btn btn-secondary" data-toggle="modal" data-target="#recent_transactions_modal" id="recent-transactions"> <i class="fas fa-clock"></i> Recent Trans.</button>
	</div>
	@endif
	@endif
