...
class SubscriptionController extends BaseController
{
    ...
    public function confirm($package_id, Request $request)
    {
        ...
        try {
            ...
            if($payment_transaction_id or true){
                ...
                $output = ['success' => 0, 'msg' => "Please try again for Payment. Thanks!"];
            }
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    /**
     * Pay with kanoo payment method - redirect to paypal url for payments
     *
     * @return Response
     */
    public function kanooExpressCheckout(Request $request, $package_id)
    {
        ...
        $return_url = action('\Modules\Superadmin\Http\Controllers\SubscriptionController@confirm', [$package->id]);
        $cancel_url = action('\Modules\Superadmin\Http\Controllers\SubscriptionController@pay', [$package_id]);
        $invoice_id = str_random(5);    
        $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,"https://api.kanoopays.com/visipay/api/external/payment/request/token");
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS,
                        "authenticationKey=24047D6D96AE428788041E125264E2E8&userId=32674&amount=".(float)$package->price."&orderId".$package->id."&redirectUrl=".$return_url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);

            $server_output = curl_exec($ch);
            $server_output = json_decode($server_output,true);
            curl_close ($ch);
            $kan =  '<a href="https://external.kanoopays.com/pay/login/'.$server_output['result'].'">Pay with Kanoo</a>';
            //
        return redirect()->away('https://external.kanoopays.com/pay/login/'.$server_output['result']);
    }
    /**
