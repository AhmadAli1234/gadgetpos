...
class CashRegisterUtil extends Util
{
    ...
    public function getInitialRegisterDetails($id)
    {
        $initial_amount = CashRegisterTransaction::where('cash_register_transactions.cash_register_id', $id)
                ->where('cash_register_transactions.transaction_type', 'initial')
                ->select(
                'cash_register_transactions.amount'    
                )
                ->first();
        return $initial_amount;         
    }
    
    public function getOnlyRegisterTransactionDetails($user_id, $open_time, $close_time)
    {
        $only_transaction_details = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transaction_date', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.is_direct_sale', 0)
                ->where('transactions.status', 'final')
                ...
                ->first();
        return $only_transaction_details;         
    }
    
    public function getSellReturnTransactionDetails($business_id, $user_id, $open_time, $close_time)
    {
        $sell_return_details = Transaction::where('created_by', $user_id)
                ->whereBetween('transaction_date', [$open_time, $close_time])
                ->where('type', 'sell_return')
                ->where('status', 'final')
                ->where('is_direct_sale', 0)
                ->select(
                    DB::raw('SUM(final_total) as total_sell_details')
                )
                ->first();
        return $sell_return_details;        
    }
    
    public function getOpenPayoutDetails($business_id, $user_id, $open_time, $close_time)
    {
        $registerDetails = DB::table('cash_registers')
            ->where('business_id', $business_id)
            ->where('user_id', $user_id)
            ->where('created_at', $open_time)
            ->where('status', 'open')
            ->first();
        if(isset($registerDetails) && $registerDetails !="") {
            $register_id = $registerDetails->id;
            if(isset($register_id) && $register_id !="") {
                $payoutAmount = DB::table('payouts')->where('cash_register_id', $register_id)->sum('amount');
    ...
    public function getClosePayoutDetails($business_id, $user_id, $open_time, $close_time)
    {
        $registerDetails = DB::table('cash_registers')
            ->where('business_id', $business_id)
            ->where('user_id', $user_id)
            ->where('created_at', $open_time)
            ->where('closed_at', $close_time)
            ->first();
        if(isset($registerDetails) && $registerDetails !="") {
            $register_id = $registerDetails->id;
            if(isset($register_id) && $register_id !="") {
    ...
    public function getSinglePayoutDetails($business_id, $user_id, $open_time, $close_time)
    {
        $registerDetails = DB::table('cash_registers')
            ->where('business_id', $business_id)
            ->where('user_id', $user_id)
    ...
    public function getPayoutTransactionDetails($business_id, $user_id, $open_time, $close_time)
    {
