...
class HomeController extends Controller
{
    ...
    public function findingg(){
     
     
                     
                     try{
                         
                     $last_sub=Subscription::where('id','<>',287)->orderBy('id','DESC')->where('business_id',session()->get('user.business_id'))->limit(1)->get();
                         
                     $sub_edit=Subscription::findOrFail($last_sub->toArray()[0]['id']);
                
                
                    $sub_edit->end_date = date("Y-m-d",strtotime("-1 days"));
                    $sub_edit->trial_end_date = date("Y-m-d",strtotime("-1 days"));
                    // $sub_edit->save();
                    echo '<pre>';
                    print_r($sub_edit->toArray());
                     }
                     catch(Exception $e){
