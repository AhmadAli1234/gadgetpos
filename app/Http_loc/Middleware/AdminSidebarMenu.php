    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('superadmin.access_package_subscriptions') && auth()->user()->can('business_settings.access')) {
                $menu->url(action('\Modules\Superadmin\Http\Controllers\SubscriptionController@index'), __('lang_v1.subscriptions'), ['icon' => 'fa fas fa-recycle', 'active' => request()->segment(1) == 'subscription'])->order(8);
            }
...
/*                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
 ...
 /*                               if (auth()->user()->can('sell.view')) {
            ...
            //Payout dropdown
            if (in_array('payouts', $enabled_modules) && (auth()->user()->can('all_payout.access') || auth()->user()->can('view_own_payout'))) {
                $menu->dropdown(
                    __('expense.payouts'),
                    function ($sub) {
                        $sub->url(
                            action('PayoutController@index'),
                            __('lang_v1.list_payouts'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'payouts' && request()->segment(2) == null]
                        );

                        // if (auth()->user()->can('payout.add')) {
                        //     $sub->url(
                        //         action('PayoutController@create'),
                        //         __('expense.add_payout'),
                        //         ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'payouts' && request()->segment(2) == 'create']
                        //     );
                        // }
                    },
