...
class ReportController extends Controller
{
     ...
     * @return void
     */
    public function __construct(TransactionUtil $transactionUtil, ProductUtil $productUtil, ModuleUtil $moduleUtil, CashRegisterUtil $cashRegisterUtil)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($registers)
                ...
                ->editColumn('closing_amount', function ($row) {
                    if ($row->status == 'close') {
                        //$ini_amount = 0;
                        //$initial_amount = $this->cashRegisterUtil->getInitialRegisterDetails($row->id);
                        $register_details =  $this->cashRegisterUtil->getRegisterDetails($row->id);
                        $details = $this->cashRegisterUtil->getOnlyRegisterTransactionDetails($row->user_id, $row->created_at, $row->closed_at);
                        $sell_return = $this->cashRegisterUtil->getSellReturnTransactionDetails($row->business_id, $row->user_id, $row->created_at, $row->closed_at);
                        $payout = $this->cashRegisterUtil->getClosePayoutDetails($row->business_id, $row->user_id, $row->created_at, $row->closed_at);
                        if($sell_return->total_sell_details !=null) {
                            $sell_return_amount = $sell_return->total_sell_details;
                        }
                        else {
                            $sell_return_amount = 0;    
                        }
                        // if(isset($initial_amount->amount) && $initial_amount->amount !="") {
                        //     $ini_amount = $initial_amount->amount;    
                        // }
                        ...
                        $credit_sales = $details->total_sales - $register_details->total_sale;
                        
                        $finalAmount = ($register_details->cash_in_hand + $register_details->total_cash + $register_details->total_cheque + $register_details->total_card + $register_details->total_bank_transfer + $register_details->total_advance + $register_details->total_custom_pay_1 + $register_details->total_custom_pay_2 + $register_details->total_custom_pay_3 + $register_details->total_custom_pay_4 + $register_details->total_custom_pay_5 + $register_details->total_custom_pay_6 + $register_details->total_custom_pay_7 + $register_details->total_other) - $sell_return_amount + $credit_sales - $register_details->cash_in_hand - $payoutAmount;
